...
export const WallpaperCanvas = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  // Background style
  const backgroundStyle: React.CSSProperties = background.type === 'image' && background.imageUrl 
    ? { backgroundImage: `url(${background.imageUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' }
    : {};

  const backgroundClass = background.type === 'gradient' ? background.gradient : 'bg-black';
    ...
    >
      ...
      <div className="relative z-10 grid grid-cols-12 grid-rows-12 h-full w-full gap-4">
        ...
        */}
        ...
        <div className="col-start-9 col-span-4 row-start-2 row-span-6 bg-black/20 backdrop-blur-sm rounded-xl p-8 border border-white/10 flex flex-col shadow-2xl">
